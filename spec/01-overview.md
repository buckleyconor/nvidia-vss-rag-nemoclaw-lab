...
**Inputs:** `concept.md` (approved) + `sizing.md` (approved) + the project build document (`NVIDIA-Service-BP-VSS-RAG-NemoClaw-plan.md`). Every vendor fact in this spec traces to those three; no company/product research profile was loaded, and no vendor claim goes beyond the build document.
...
- **Runtime & deployment target:** one Ubuntu 24.04 x86 vCD VM per learner, Docker Compose only — **no Kubernetes** (VSS ships Compose-only; its `native.cgroupdriver=cgroupfs` daemon requirement conflicts with MicroK8s). vRAM on the 96 GB card is the binding constraint: one instance per card; the 10-VM pool (hard cap 10 concurrent learners) bounds N.
...
- **Hard constraints:** GPU budget ~70 GB committed of 96 GB (~26 GB headroom — estimates pending the build document's Phase 2 measurement); RAM 256 GB (~224 GB committed + 32 GB /dev/shm); disk 2 TB (~1.65 TB committed); the non-negotiable start order (shim → VSS on empty GPU → gate on RT-VLM → RAG → NemoClaw); vendor floors (NVIDIA driver 580.105.08 — the VSS canonical-matrix exact pin for Ubuntu 24.04, Docker ≥ 28.3.3 **and < 29.5.0** — newer Docker breaks NGC pulls, Compose ≥ v2.39.1, `vm.max_map_count=262144`, `/dev/shm` 32 GB, full PCIe passthrough not vGPU); exact pinned versions, never `latest`.
...
4. State why the reasoning LLM is shared/off-VM while the video model and the RAG retrieval models run locally on the GPU — the footprint/density reason: one VM, one 96 GB GPU, a pool of ten learners.
