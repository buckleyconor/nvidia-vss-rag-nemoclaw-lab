...
|---|---|---|
| VSS repo tag | **v3.2.1** | SHA `7640d917047cf7b0fd3085eefb8282754b56bc94` |
...
The one distinction still worth holding: the **repo tag** is what you
`git clone -b`; **`VSS_AGENT_VERSION`** is the agent *container image* tag set
in the `.env`. They version independently. A mismatch is a prep finding to
surface, never a silent substitution.

Never use `latest` for anything in this build.
...
VLM NIM. This resolves the earlier open question in favour of the current-docs
answer rather than the repo README's `Cosmos-Reason2-8B`. It is 8B-class, so
the memory budget below is unaffected. Do not substitute — LVS prompts and
alert verification are tuned around this image.

The same release lists `nvcr.io/nim/nvidia/nemotron-3-nano:1` as VSS's local
LLM NIM. **It must not run here** — the LLM role goes to the shared endpoint.
A live NIM on :30081 is a misconfiguration signal.

**Exact local NIM image tags:**

```
nvcr.io/nim/nvidia/cosmos3-reasoner:1.7                  # VSS VLM
nvcr.io/nim/nvidia/llama-nemotron-embed-1b-v2:1.13.0     # RAG
nvcr.io/nim/nvidia/llama-nemotron-rerank-1b-v2:1.10.0    # RAG
nvcr.io/nim/nvidia/nemotron-page-elements-v3:1.8.0       # RAG
nvcr.io/nim/nvidia/nemotron-graphic-elements-v1:1.8.0    # RAG
nvcr.io/nim/nvidia/nemotron-table-structure-v1:1.8.0     # RAG
nvcr.io/nim/nvidia/nemotron-ocr-v1:1.3.0                 # RAG
```

Note the OCR image is `nemotron-ocr-v1`, not `nemotron-ocr`. Prep records the
actually pulled tags; any mismatch is a finding to surface.
...
| Docker | **≥ 28.3.3 and < 29.5.0** |
| Docker Compose | **≥ v2.39.1** |
| NGC CLI | 4.10.0+ |
| Node.js | 20+ (NemoClaw requirement) |

> **The Docker upper bound is real.** Docker ≥ 29.5.0 breaks NGC image pulls.
> This is the one version constraint in the table with a ceiling as well as a
> floor, and installing "latest Docker" on a fresh VM will overshoot it. An
> earlier draft of this document said 27.2.0+ / Compose 2.29.0+ with no ceiling;
...
```bash
# Docker must land in [28.3.3, 29.5.0) — NGC pulls break above the ceiling.
# get.docker.com installs the newest release, which will overshoot. Pin it.
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh --version 28.3.3
...
docker compose version   # must be >= 2.39.1
docker --version         # must be >= 28.3.3 AND < 29.5.0
```
...
curl -X POST http://localhost:8082/v1/documents \
  -F "documents=@/data/corpus/manual-01.md" \
...
COMPATIBLE_API_KEY='dummy' \
  bash deploy/docker/scripts/nemoclaw/init_nemoclaw.sh \
    --sandbox-name demo \
    --policy-file /path/to/nemoclaw-policy.yaml
```

The sandbox name is a flag, not a positional argument. The policy file is the
VSS preset (`assets/vss_nemoclaw_policy.yaml` in the VSS checkout) extended with
the §9.2 endpoints; `scripts/prep/40-nemoclaw.sh` generates it.
...
**No RAG entry.** The agent's only knowledge path is the VSS frag tool
(agent → VSS → RAG), so the sandbox never calls RAG directly. A policy entry
for 8081 is unnecessary, and on the `install-vllm` onboarding path it
duplicates the local-inference policy that already claims 8081 (VSS PR #2027).
See `operator-dashboard-spec.md` ADR-V06.
...
| Item | Resolve by |
|---|---|
| Verify `VSS_AGENT_VERSION=3.2.1` exists on nvcr.io and is referenced by the v3.2.1 release compose | Before cloning |
...
| ~~Choose downstream action target~~ — mock-wo, gated by the operator dashboard (`operator-dashboard-spec.md`) | Closed |
