...
| 4 | **NemoClaw** | VSS-repo installer `deploy/docker/scripts/nemoclaw/init_nemoclaw.sh` (fresh OpenClaw required; Node 20 host requirement; the NemoClaw source checkout **v0.0.118 → `~/NemoClaw`** is a pre-condition — the installer runs its `install.sh`, cloned by `10-clone`, 2026-09-10 dry-run) | OpenClaw sandbox + OpenShell gateway (**server/health/metrics :8085/:8086/:8087** — the vendor default 8080 collides with the auth-shim, 2026-09-10 dry-run); dashboard forward :18789; reads the `vss-generate-video-report-rag` skill; HITL kick-off; drives VSS + RAG; performs the downstream action (POST to mock-wo) | 3 (visible tool calls/reasoning), 4 (work order), 5 (triage) |
...
| 7 | **Shared off-VM LLM endpoint** | Pre-provisioned, **not on the VM, not in the GPU budget** | `NVIDIA/Nemotron-3.5-Lightning-30B-A3B` at `https://model.delllabs.local/api/nemotron35/v1` (owner-confirmed 2026-09-07; served as the NVFP4 deployment id `nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4` — recorded 2026-09-10) via auth-shim; serves VSS LLM role + RAG generation + NemoClaw (replaces `nemotron-3-super-120b-a12b`, `nemotron-nano-9b-v2`, and NemoClaw's default model) | All LLM roles |
...
**Model placement** (build doc §1): local GPU = VSS VLM `nvcr.io/nim/nvidia/cosmos3-reasoner:1.7` (Cosmos3 Nano Reasoner — VSS 3.2.1 release default) + the six retrieval NIMs `nvcr.io/nim/nvidia/llama-nemotron-embed-1b-v2:1.13.0`, `nvcr.io/nim/nvidia/llama-nemotron-rerank-1b-v2:1.10.0`, `nvcr.io/nim/nvidia/nemotron-page-elements-v3:1.8.0`, `nvcr.io/nim/nvidia/nemotron-graphic-elements-v1:1.8.0`, `nvcr.io/nim/nvidia/nemotron-table-structure-v1:1.8.0`, `nvcr.io/nim/nvidia/nemotron-ocr-v1:1.3.0` (all user-supplied from the blueprint release data, 2026-09-02) = 7 compute processes at steady state, ~75 GB committed / ~21 GB headroom (measured on the H100 at prep, 2026-09-10: 75365/96256 MiB — 09; NIM profile selection is GPU-dependent). Shared endpoint = `NVIDIA/Nemotron-3.5-Lightning-30B-A3B` (30B A3B) serving all three LLM roles. Measured run split (~250 s e2e): VSS video analysis ~92%, NemoClaw ~5.7%, RAG retrieval ~1.3% (~1.69 s), LLM fusion ~1.0% (~1.24 s) — the VLM is the whole cost; the off-VM LLM is latency-neutral.
...
```
                ...
                (https://model.delllabs.local/api/nemotron35/v1)
                               ...
                               │
┌────────────────────────────────────────────────────────────────────────┐
│ vCD VM — Ubuntu 24.04 · 32 vCPU · 256 GB · 2 TB · 1× H100 ~94 GB       │
│                                                                        │
...
| 5 | Start **NemoClaw sandbox** (`init_nemoclaw.sh`, fresh OpenClaw; re-run path supported — 08 item 42) | `40-nemoclaw` gate: host-side `nemoclaw demo status` (model + compatible-endpoint + healthy inference + `vss` policy) + sandbox-side `openclaw nemoclaw status` registration box + `:8080/v1/models` via the shim (08 item 43) |
...
| LVS backend | 38111 | `/v1/ready` | start gate, learner verification | LVS mode: `MODE=2d`, `BP_PROFILE=bp_developer_lvs`, `HARDWARE_PROFILE=H100`, `LLM_MODE=remote`, `VLM_MODE=local_shared`. **LVS UI port/URL not recorded in the build document — release-dependent, recorded at prep (§8)** |
...
| RAG ingestor | 8082 | `PATCH /v1/documents` (multipart: `documents=@file`, `data={"collection_name":"demo_corpus","blocking":true}`; rework 2026-09-14 #18/#12 — at v2.6.2 a missing collection ⇒ HTTP 500 (`main.py:415`), a non-blocking POST returns 200 + task_id before ingestion runs (`server.py:330`), and a POST re-upload of an existing document fails as "already exists" **inside an HTTP 200 body** (`main.py:637`) — PATCH replaces an existing document, so the loop judges `failed_documents`/`validation_errors` and re-runs are idempotent; the collection is created at prep if absent) | **prep-time only** — the learner never runs ingestion | collection name `demo_corpus` = `KNOWLEDGE_COLLECTION` |
