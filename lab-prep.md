---
baseline: 'vCD VM: Ubuntu 24.04 x86, 32 vCPU / 256 GB RAM / 2 TB NVMe, 1x H100 ~94 GB (vGPU partition, SKU H100L-94C — learner SKU, platform-confirmed 2026-09-07), /dev/shm 32 GB'
software:
...
endpoints:
  ...
  - { url: 'https://model.delllabs.local/api/nemotron35/v1 (shared off-VM inference endpoint, pre-provisioned; reached via the auth-shim — the shim lab.env value is the prefix before /v1: https://model.delllabs.local/api/nemotron35/)', purpose: 'NVIDIA/Nemotron-3.5-Lightning-30B-A3B serving all LLM roles (VSS LLM, RAG generation, NemoClaw)' }
artifacts:
...
- Image / OS: vCD VM — Ubuntu 24.04 x86, 32 vCPU / 256 GB RAM / 2 TB NVMe, 1× H100 ~94 GB (vGPU partition, SKU H100L-94C — learner SKU, platform-confirmed 2026-09-07), /dev/shm 32 GB.
...
| `https://model.delllabs.local/api/nemotron35/v1` — shared off-VM inference endpoint (pre-provisioned, via the auth-shim) | `NVIDIA/Nemotron-3.5-Lightning-30B-A3B` serving all LLM roles |
